...
class QuizCustomListCrud:
    ...
    async def get_quiz_list_custom_by_id(self,id:int)->list[QuizCustomListReadByIdSchema]:
        ...
        """
        query=select(QuizCustomListModel.c.id,QuizCustomListModel.c.Genre,QuizCustomListModel.c.Description).select_from(QuizCustomListModel).where(QuizCustomListModel.c.User_ID==id)
